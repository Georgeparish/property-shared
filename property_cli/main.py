...
def analysis_yield(
    ...
    if http:
        ...
        table.add_row("Assessment", classify_yield(result.gross_yield_pct) if result.gross_yield_pct is not None else "—")
        table.add_row("Data Quality", classify_data_quality(result.sale_count, result.rental_count))
        rprint(table)
...
def analysis_rental(
    ...
    if http:
        ...
        if result.gross_yield_pct is not None:
            ...
            table.add_row("Assessment", classify_yield(result.gross_yield_pct))
        rprint(table)
...
def report_generate(
    ...
    # Key insights (generated by consumer, not core)
    from property_core.interpret import estimate_value_range, generate_insights
    ...
    # Value estimate (generated by consumer, not core)
    if report_data.market_analysis and report_data.market_analysis.median_price:
        low, high = estimate_value_range(report_data.market_analysis.median_price)
        rprint(f"\n[bold]Estimated Value Range:[/bold] £{low:,} - £{high:,}")
